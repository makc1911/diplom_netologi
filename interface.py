...
from data_store import add_user, check_user, engine
    ...
    def event_handler(self):
        ...
        for event in longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                if command == 'привет':
                    self.params = self.api.get_profile_info(event.user_id)
                    if self.params['city'] is None and (self.params['age'] is None or self.params['age'] == 0):
                    ...
                    elif self.params['age'] is None or self.params['age'] == 0:
                        self.message_send(event.user_id, f'Здравствуйте, {self.params["name"]} введите, '
                    ...
                    if self.params['city'] is None and self.new_city == ['empty']:
                        self.message_send(event.user_id,
                                          f'Нет города, укажите пожалуйста ваш город, в формате: город Москва')
                    elif self.params['age'] is None and self.new_age == 0:
                        self.message_send(event.user_id,
                                          f'Нет возраста, укажите пожалуйста ваш возраст в формате: мне 25')
                    else:
                        self.params['city'] = self.new_city if self.params['city'] is None else self.params['city']
                        self.params['city_id'] = self.new_city_id
                        self.params['age'] = self.new_age if self.params['age'] is None or self.params['age'] == 0 \
